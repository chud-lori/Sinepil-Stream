...
const { movieStmts, indexMovies } = require('../db');
...
const SOURCE_REFERER = 'https://tv10.lk21official.cc/';
...
async function getMovie(slug) {
  ...
  const players = await resolvePlayers(rawPlayers, { referer: SOURCE_REFERER, origin: SOURCE_ORIGIN });
...
async function getRating(slug) {
  try {
    const { $ } = await fetchHtml(`${BASE}/${slug}/`);
    const ld = parseJsonLd($);
...
module.exports = {
  ...
  getMovie, search, browse, seedIndex, slugFromSourceUrl,
};
