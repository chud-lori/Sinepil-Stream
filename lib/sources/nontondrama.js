...
async function getSeries(slug) {
  ...
  const title = (meta.title || ld.name || slug).toString().trim();
    ...
    .map(k => ({
      ...
      episodes: (Array.isArray(seasonData[k]) ? seasonData[k] : [])
        .map(e => ({
          ...
          title: (e.title || '').replace(/^[^-]+-\s*/, '').replace(/\s*\(\d{4}\)\s*$/, '').trim(),
          slug: String(e.slug || ''),
        }))
  ...
  // Cache metadata for search fallback
  indexSeries([{
    slug,
    title: title.replace(/^Lk21\s+Nonton\s+/i, '').replace(/\s+Series\s+Sub\s+Indo.*$/i, '').replace(/\s*\(\d{4}\)\s*$/, '').trim() || title,
    ...
    genre: Array.isArray(ld.genre) ? ld.genre.join(', ') : (ld.genre || ''),
  ...
  return {
    kind: 'series',
    slug,
    title: (meta.title || title).replace(/^Lk21\s+Nonton\s+/i, '').replace(/\s+Series\s+Sub\s+Indo.*$/i, '').replace(/\s*\(\d{4}\)\s*$/, '').trim() || title,
    poster: meta.poster || ld.image?.url || ld.image || $('meta[property="og:image"]').attr('content') || '',
    description: ld.description || $('meta[property="og:description"]').attr('content') || $('meta[name="description"]').attr('content') || '',
    genre: Array.isArray(ld.genre) ? ld.genre.join(', ') : (ld.genre || ''),
    year: String(meta.year || ld.datePublished || '').slice(0, 4),
    rating: String(meta.rating || ld.aggregateRating?.ratingValue || ''),
    director: Array.isArray(ld.director) ? ld.director.map(d => d.name).join(', ') : (ld.director?.name || ''),
    cast: (ld.actor || []).slice(0, 8).map(a => a.name).join(', '),
    seasons,
